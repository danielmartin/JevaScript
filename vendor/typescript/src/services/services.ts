    ...
    while (pos < end) {
        ...
        if (textPos <= end) {
            if (token === SyntaxKind.Identifier) {
                ...
                // JevaScript lowers these modifiers before binding. Keep their
                // source tokens available to editor navigation after lowering.
                if (parent.getSourceFile().fileName.endsWith(".jeva.ts") && ["threshold", "and"].includes(scanner.getTokenValue())) {
                    const modifier = createNode(token, pos, textPos, parent) as IdentifierObject;
                    modifier.escapedText = escapeLeadingUnderscores(scanner.getTokenValue());
                    nodes.push(modifier);
                    pos = textPos;
                    continue;
                }
