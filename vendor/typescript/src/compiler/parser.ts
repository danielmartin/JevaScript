...
import { decisionKind, lowerJevaSourceFile } from "./jeva.js";
...
namespace Parser {
    ...
    function parseSourceFileWorker(languageVersion: ScriptTarget, setParentNodes: boolean, scriptKind: ScriptKind, setExternalModuleIndicator: (file: SourceFile) => void, jsDocParsingMode: JSDocParsingMode): SourceFile {
        ...
        let sourceFile = createSourceFile(fileName, languageVersion, scriptKind, isDeclarationFile, statements, endOfFileToken, sourceFlags, setExternalModuleIndicator);
        ...
        if (fileName.endsWith(".jeva.ts")) sourceFile = lowerJevaSourceFile(sourceFile);
    ...
    function parseExpressionOrLabeledStatement(): Statement {
        ...
        const expression = allowInAnd(parseExpression);
        const decision = fileName.endsWith(".jeva.ts") ? decisionKind(expression) : undefined;
        if (token() === SyntaxKind.OpenBraceToken && (decision === "test" || decision === "route" || decision === "threshold" || decision === "and")) {
            if (decision === "route") {
                return withJSDoc(finishNode(factory.createSwitchStatement(expression, parseCaseBlock()), pos), hasJSDoc);
            }
            const yes = parseBlock(false);
            const no = parseOptional(SyntaxKind.ElseKeyword) ? parseStatement() : undefined;
            return withJSDoc(finishNode(factoryCreateIfStatement(expression, yes, no), pos), hasJSDoc);
        }
