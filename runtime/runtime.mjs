...
function start() {
  ...
  starting = new Promise((resolve, reject) => {
    const vela = process.env.JEVA_VELA_DIR ?? path.resolve(root, '../vela-decision');
    const python = process.env.JEVA_PYTHON ?? path.join(vela, '.venv/bin/python');
    const model = process.env.JEVA_MODEL ?? path.join(vela, 'work/candidate');
    if (!fs.existsSync(python) || !fs.existsSync(path.join(model, 'model.safetensors'))) {
      reject(new Error('Vela is not installed. Keep vela-decision beside JevaScript, or set JEVA_VELA_DIR / JEVA_PYTHON / JEVA_MODEL.'));
      return;
    }
    ...
    worker = spawn(python, ['-u', path.join(root, 'runtime/worker.py'), '--port', '0', '--model', model], {
      ...
      env: { ...process.env, PYTHONPATH: [vela, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter),
    ...
    let stderr = '';
    let ready = false;
    const timer = setTimeout(() => { child.kill(); reject(new Error('Vela did not become ready within 120 seconds')); }, 120_000);
      ...
      while ((newline = buffer.indexOf('\n')) !== -1) {
        ...
        if (line.startsWith('JEVA_READY ')) {
          try {
            ...
            if (!Number.isInteger(port) || port <= 0 || port > 65535) throw new Error('Invalid Vela worker port');
            ready = true;
            clearTimeout(timer);
            trace(`Vela ready in ${(performance.now() - started).toFixed(0)} ms`);
    ...
    child.once('exit', (code, signal) => {
      ...
      starting = undefined;
      if (!ready) reject(new Error(`Vela startup failed (${signal ?? code}): ${stderr.trim()}`));
      else if (code && process.env.JEVA_TRACE === '1') trace(`Vela exited: ${stderr.trim()}`);
    });
  });
